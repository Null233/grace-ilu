...
class _DistributedOptimizer(torch.optim.Optimizer):
    ...
    def _allreduce_grad_async(self, p):
        ...
        tensor_compressed, ctx, shape = self._compression.compress(tensor, name)
    ...
    def _grouped_allreduce_grad_async(self, ps):
        ...
        tensors_compressed, ctxs, shapes = zip(*[self._compression.compress(p.grad, name) for p in ps])
            ...
            if isinstance(p, tuple):
                ...
                if self._groups is not None:
                    ...
                    if self._group_counts[group] != 0:
                        self._group_counts[group] = 0
                if p.grad.is_sparse:
                    aggregated = self._compression.decompress(output, ctx, self._parameter_names.get(p))
...
class _DistributedAdasumOptimizer(torch.optim.Optimizer):
    ...
    def _allreduce_grad_async(self, p):
        ...
        # allreduce as before
        tensor_compressed, ctx, shape = self._compression.compress(p, name)
    ...
    def step(self, closure=None):
        ...
        for p, (handle, ctx) in self._handles.items():
            ...
            delta = self._compression.decompress(delta, ctx, self._parameter_names.get(p))
            start = self._starting_models[p]
